<!-- GENERATED FILE - do not hand-edit. Source: templates/architecture.template.md + CLAUDE.md. Regenerate: node scripts/generate-alex.js. Generated 2026-09-24. -->
...
Alex is the owner's personal AI agent, not a chatbot. Three things make it more than a chat window: it remembers (a persistent Obsidian vault of people, projects, business, decisions, the owner's own words), it acts (email drafts, calendar, documents, job pipelines, with guardrails), and it runs on a schedule (the laptop wakes and works, nobody presses a button). Under the hood it is Claude running inside Claude Code, wrapped in a folder of files, rules, and schedules. That wrapper is this repo.
...
Never ask, never offer, never close a session with "want me to commit?". the nightly git-backup job commits and pushes on its own (`scripts/git-backup.ps1` on Windows, `run-job.mjs git-backup` on macOS). Report what changed and where; do not hand back a git decision. The order suppresses the ASK, not the judgment, and three things stay yours to act on:
- **Privacy, and this one is urgent.** `.gitignore` is the ONLY barrier between the vault and the internet. Anything new holding personal data, credentials or vault content must be gitignore-covered BEFORE it is ever committed, and `git check-ignore <path>` is how you prove it rather than assume it. Say it the moment you see it.
- **A tree that would fail the gate** (the pre-commit hook runs the validators, the secret scan and the donor-identity scan): fix it before close-out; do not report it as a question.
...
Alex only exists when Claude Code's session folder IS this folder, the one containing `CLAUDE.md` and `soul.md`. This constitution, every slash command, and the hooks in `.claude/settings.json` are downstream of that fact. Attaching files, dragging the folder in, or pasting a path does NOT load Alex.

- **Open the folder, do not attach it.** In VS Code: File, then Open Folder, then pick the Alex folder itself. From a terminal: `cd` into it, then run `claude`. A SUBFOLDER does not count, and neither does dragging files in.
...
| 18 | (no command) | LIVE | logon (staggered 20/25/30 min) | Backups (git plus an encrypted vault tar to your own Drive), the zero-token drift checker, the status rotation caps, and the Recall Spine fact ledger. This is the layer that notices when something quietly stopped working. | work/18-recovery-layer - vault/projects/recovery/status.md |
...
**114 third-party skills** live at `.agents/skills/` (the real content, in the repo) with junctions in `.claude/skills/` (the discovery layer, gitignored). Which of them are awake and which are parked depends on this machine's install profile, so no number is written here: `node scripts/skills-park.js --list` prints the split. Parking keeps a skill's content but removes its junction, so it stops loading without being deleted. Wake one with `node scripts/skills-park.js --wake <name>`.
...
**On restore, the skill links need rebuilding**, because they are gitignored links rather than files: `node scripts/bootstrap.mjs --repair-links` (one command, every platform - junctions on Windows, symlinks on macOS; it also applies this machine's install profile). Do this BEFORE running the validators, or V17 fails on links that do not exist yet. Never hand-build them with `ln -s` in Git Bash on Windows: it silently COPIES instead of linking.
...
When the owner asks to schedule something: add it to `scheduler/schedule.md` with a `- Frequency:` and a `- Script:` line, then register it with `node scripts/generate-alex.js --only=scheduler` (add `--dry-run` first to see what it would do). `/cron-setup` walks the same ground conversationally.

**Everything triggers on sign-in with a delay, not at a set hour.** The reasoning is in `scheduler/schedule.md` and it comes down to this: a laptop is not reliably awake at 05:00, and a job that never runs looks exactly like a day where nothing happened.

**Two scheduled jobs spend Claude allowance**: `/email-triage` on every sign-in, and `/self-review` once a month. Everything else is a zero-token script. On a Pro plan the allowance is the binding constraint, and a background job quietly eating it is the failure nobody diagnoses because the symptom is "Alex stopped working this afternoon". Before scheduling anything new that calls Claude, say what it will cost and what it displaces.

## Backup & Recovery

Two backups, and they cover different things on purpose.

**1. The system, to GitHub** (the nightly git-backup job: `scripts/git-backup.ps1` on Windows, `run-job.mjs git-backup` on macOS). Code, specs, configuration. The repo is **PRIVATE** and must stay private. The vault, `soul.md`, credentials and personal files are gitignored and never go with it. Sign-in is through Git Credential Manager: a browser window, no token to copy or keep.

**2. The vault, encrypted, to the owner's own Google Drive** (`scripts/vault-backup.ps1` on Windows, `scripts/vault-backup.mjs` on macOS). This is the one that matters if the laptop dies. It tars everything git deliberately ignores, encrypts it with gpg AES256 using the owner's own passphrase, **decrypts it again to prove it can be opened**, then ships it with rclone and keeps the last 14. Google holds ciphertext only.

**The operating rules, all load-bearing:**
1. `.gitignore` is the SOLE barrier between the vault and the internet. Never `git add -f` a gitignored path.
...
3. **The passphrase is the whole ballgame.** It lives in the owner's password manager AND in a local file outside this folder. **If it is lost, every backup is permanently unreadable, and nobody can recover it** - not the person who set this up, not Google, not Anthropic. `/setup` refuses to finish until it is stored in two places, and check C14 asks the owner to re-confirm it every 90 days.
4. Prove the restore rather than trusting it: `powershell -File work/18-recovery-layer/escrow-test.ps1` pulls the newest backup back down and decrypts it with the passphrase from the password manager. Run it on install day, not "later".
5. The repo must never be made public. The monthly security sweep (S8) asks GitHub directly and reports a mismatch.
...
|---|---|---|---|---|
| 1 | `git push` to the owner's own repository | the whole tree, on every autosave | the cloud proxy scopes the credential to that one repository; the pre-commit content legs (secret scan, size guard, employer-data guard) run on every commit | the commit history itself, and `outputs/logs/autosave.log` |
