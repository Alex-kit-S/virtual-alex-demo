---
name: no-ai-slop
description: Detect and name AI-slop patterns in a draft (colon reveals, faux-insight setups, importance puffery, weasel attribution, synonym cycling, metadiscourse, fake-profound kickers), or edit a draft to remove them while preserving the writer's voice. Use when a draft reads as AI, when auditing prose before it ships, or when the owner asks whether writing sounds like AI. DETECT mode is the primary use; soul.md always outranks this file on any conflict.
---

<!-- ============================ ALEX SCOPE GUARD ============================
Added locally at install (2026-08-21). This file is a DATA reference installed under
Alex's DATA-only posture. Upstream: petergyang/no-ai-slop @ d30eddb, MIT.
The computedHash in skills-lock.json intentionally differs from upstream because of
this block and the rewritten frontmatter description.

soul.md IS THE CONSTITUTION FOR VOICE. This skill is a pattern checklist underneath it.
Where the two disagree, soul.md wins, every time, with no exceptions. Four overrides:

1. EM DASHES: this file says "1-2 are fine in longer drafts". WRONG HERE. The rule in
   CLAUDE.md is ZERO em dashes and ZERO en dashes, in every piece of prose, and
   brand/config/writing-style.md rule 5 says the same for analytical output. There is no
   draft length at which one becomes acceptable. Never relax it on this file's authority.

2. A LOCKED FORMAT OUTRANKS THIS FILE. If the owner has locked a recurring format (a post
   template, a report shape, a client-facing letter structure) and that format deliberately
   uses something listed here as slop, the format wins inside that format and this file is
   ignored on those points. The usual collisions are binary contrasts ("it's not X, it's Y"),
   a deliberate one-line-per-thought layout, and a closing signature line. Outside that
   format the bans apply normally. When in doubt, ask the owner rather than editing a
   template they chose.

3. VOICE COMES FROM THE CORPUS, NOT FROM THE DRAFT. This file infers voice from the single
   draft in front of it ("identify 3-5 voice signals"). Alex does not work that way: the
   voice source is soul.md My Words, and the Brand + Soul Pre-Flight Gate requires reading it
   before generating. Read the corpus first; use this file only to name patterns in the output.

4. THE OWNER'S IMPERFECTIONS ARE THE SIGNAL, NOT ERRORS. This file never says so, and a
   generic sharp editor "fixes" them. The standing rule in CLAUDE.md is the opposite: a
   dropped article, a run-on, an unusual word order, a favourite filler word are PRESERVED.
   Never correct the owner's grammar into polished English on this file's authority. That
   correction is the single fastest way to make their text read as machine-written. Note the
   one legitimate exception, already written down: brand/config/writing-style.md governs
   analytical third-person output (papers, reports) and explicitly does NOT carry the
   register half across. Owner's voice: keep the texture. Research prose: do not import it.

Also: this file's "ask the owner to paste a draft" and "ask one question about audience" steps
are fine in a conversation, but never block an unattended run on them; a scheduled run uses
what it has.
========================================================================== -->
