...
**in the owner's own voice** (email, LinkedIn, cover letters). This file governs prose
...
A noun phrase, a colon, then a small dramatic reveal. *The detail that makes it work: a
separate agent grades it.* *The best part: it learns.* Rewrite as a plain sentence.
Colons are for lists, labels and quotations, not for manufactured suspense.

### 1.10 Faux-insight setups

*What most people get wrong*, *the part everyone misses*, *what nobody tells you*,
*this is the part that gets skipped*. They flatter the writer as the lone expert and
they carry no information. Cut the setup and let the claim stand by itself.

### 1.11 Superficial analysis

Trailing `-ing` clauses that look like interpretation and supply none: *highlighting*,
*underscoring*, *reflecting*, *showcasing*, *demonstrating a commitment to*. Replace
with the actual consequence. *The release adds file search, highlighting a focus on
workflow* becomes *the release adds file search, so a user finds an old draft without
leaving the editor*.

### 1.12 Importance puffery

*Stands as a testament*, *marks a pivotal moment*, *plays a vital role*, *solidifies
its position*, *underscores the significance of*. State the fact and let the reader
decide whether it matters. Significance asserted is significance not shown.

### 1.13 Weasel attribution

*Experts agree*, *studies show*, *research suggests*, *it is widely regarded as*,
*many argue*. Name the source or cut the claim. This one is stronger here than in the
general voice rules, because an unsourced claim in analytical prose is a citation
failure and not only a style failure. See rule 3.

### 1.14 Synonym cycling

Rotating *agent*, *assistant*, *tool*, *system*, *platform* for one thing to avoid
repeating a word. In analytical writing the rotation reads as four things rather than
one, which is a comprehension cost paid for nothing. If the clear term is right,
repeat it. Consistent terminology is a feature of technical prose, not a weakness.

### 1.15 Interpretive metadiscourse

Lines that step outside the subject to tell the reader what to notice or how much
weight to give it: *the key point is*, *this distinction matters*, *as you can see*,
*that last part matters more than it sounds*, a redundant *in other words*. If the
point is already clear, delete the aside. If it is not clear, the fix is more evidence,
never a louder instruction to the reader.

### 1.16 Fake-profound kickers

The closing line that turns the argument into an aphorism, a metaphor or a mic drop.
Delete it. Do not rewrite it into a better metaphor and do not preserve its rhythm.
End on the clearest concrete sentence already present, or add a plain statement of what
follows. Pairs with 1.7: a section can fail both, once by summarising and once by
reaching for a flourish on the way out.

### 1.17 Formatting slop

Emoji in headings. Bold scattered mid-sentence for emphasis. A bulleted list where two
sentences of prose read better. A heading over a two-sentence section. Formatting
follows the content; it does not decorate it. In a document governed by an external
standard, that standard decides the formatting and this rule yields to it.
...
difference between this register and the owner's private register is formality, not
...
Rules 1.9 to 1.14 and 1.17 are phrase-shaped or list-shaped, so they are checkable the
same way and the detectors should grow to cover them. Rules 1.4, 1.6, 1.7, 1.15, 1.16,
3, 4, 6 need a reading pass, because each turns on whether the sentence is doing real
work in context and no regex can judge that.

The `no-ai-slop` skill is the on-demand reading pass for rules 1.9 to 1.17. Its DETECT
mode names the pattern, quotes the line and gives the fix without rewriting. Its own
em-dash tolerance is overridden inside the installed file; rule 5 here wins.
